...
def generate_report(user_context: dict[str, Any] | None = None) -> str:
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    with open(EVENTS_PATH) as f:
        events_data = json.load(f)

    if os.path.exists(GEMINI_ANALYSIS_PATH):
        with open(GEMINI_ANALYSIS_PATH, encoding="utf-8") as f:
            gemini_analysis = f.read()
    else:
        gemini_analysis = "No separate video analysis; report based on CV events."
    ...
    if context_block:
        prompt = f"{prompt}\n\n{context_block}\n\nPersonalize the report to the driver's goals and experience level."
